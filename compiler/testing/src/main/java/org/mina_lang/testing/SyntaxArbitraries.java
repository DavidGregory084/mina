...
public class SyntaxArbitraries {
    ...
    private static Arbitrary<FieldPatternNode<Attributes>> fieldPatternNode(GenEnvironment env, GenScope scope, String fieldName, Meta<Attributes> fieldMeta) {
        ...
        return patternNode(env, scope, fieldType).optional().map(pat -> {
            Meta<Attributes> fieldPatMeta;
            if (pat.isEmpty()) {
                var fieldPatName = new LocalName(fieldName, env.localVarIndex().getAndIncrement());
                fieldPatMeta = Meta.of(fieldPatName, fieldMeta.meta().sort());
                scope.putValue(fieldName, fieldPatMeta);
            } else {
                fieldPatMeta = Meta.nameless(fieldMeta.meta().sort());
            }
    ...
    public static Arbitrary<BinaryOp> binaryBitwiseOperators() {
        return Arbitraries.of(
    ...
    public static Arbitrary<BinaryOp> binaryRelationalOperators() {
        return Arbitraries.of(
            BinaryOp.LESS_THAN,
            BinaryOp.LESS_THAN_EQUAL,
            BinaryOp.GREATER_THAN,
    ...
    public static Arbitrary<BinaryOp> binaryNumericOperators() {
        return Arbitraries.of(
            BinaryOp.MULTIPLY,
            BinaryOp.DIVIDE,
            BinaryOp.MODULUS,
    ...
    @SafeVarargs
    public static Arbitrary<? extends ExprNode<Attributes>> binaryOpNode(GenEnvironment env, Type resultType, Type leftOpType, Type rightOpType, Arbitrary<BinaryOp> binOp, Arbitrary<BinaryOp> ...binOps) {
        return Combinators.combine(
            exprNodeWithType(env, leftOpType),
    ...
    public static Stream<Arbitrary<? extends ExprNode<Attributes>>> binaryOpNodeWithType(GenEnvironment env, Type typ) {
        ...
        } else if (typ.equals(Type.DOUBLE)) {
            return Stream.of(
